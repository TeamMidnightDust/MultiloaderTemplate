package com.example.yourmod.config;
...
public class MidnightConfigExample extends MidnightConfig {
    ...
    @Client @Entry(category = "text") public static String name = "Hello World!";                       // Example for a string option, which is in a category!
    ...
    public enum TestEnum {                                                                      // Enums allow the user to cycle through predefined options
        QUILT, FABRIC, NEOFORGE
    }
    @Entry(category = "numbers") public static int fabric = 16777215;                           // Example for an int option
    @Server @Entry(category = "numbers") public static double world = 1.4D;                             // Example for a double option
    ...
    @Entry(category = "text", name = "block.minecraft.white_concrete", isColor = true) public static String titleColor = "#ffffff";      // The isColor property adds a preview box for a hexadecimal color
    @Entry(category = "text", name = "Array List") public static List<String> arrayList = Lists.newArrayList();      // Array String Lists are also supported
    ...
    The .json language file for your config class could look similar to this:
    {
	    ...
	    "modid.midnightconfig.enum.TestEnum.NEOFORGE":"Fast",
